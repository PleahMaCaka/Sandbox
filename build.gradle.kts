    ...
    lazyToken("minecraft_classpath") {
        val joined =
            configurations.getByName("library").copyRecursive().resolve()
                .filter { !(it.absolutePath.contains("lwjgl") && it.absolutePath.contains("3.3.0")) }
...
mixin.run {
    add(sourceSets.main.get(), "mixins.sandbox.refmap.json")
    config("mixin.sandbox.json")
...
tasks.withType<Jar> {
    ...
    manifest {
        ...
        map["Specification-Title"] = "sandbox"
